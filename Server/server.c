...
/*Permet l'envoi d'un message en particulier*/
int send_mail_PARTICULAR(char *arg){
    FILE* liste = NULL; /*pointeur sur le fichier*/
	char ligne[LIMIT_CHAR*2+1] = ""; /*chaine de char accueillant la ligne courante dans le fichier*/
	...
	if(liste != NULL){ /* si l'ouverture s'est correctement passée*/
		while(!OK && fgets(ligne, LIMIT_CHAR*2+1, liste) != NULL){ /* parcours du fichier liste*/
            ...
            if(cmp_word("#!", wd_in_list, lg_wd_list)){ /*On vérifie que c'est bien une entête qui suit*/

                lg_wd_list = get_word(ligne, id_mail, 1); /*possible id_mail à envoyer*/
                numero_ligne+=1; /*incrementation du numero de ligne*/

                if(cmp_word(arg, id_mail, lg_wd_list)){/*Si les noms correspondent*/
                    /*si les id correspondent : on envoie 4 lignes contenant : id_mail/nFrom/nTo/nObj/nTexte\n*/
                    ...
                    do{
                        /*Récupérer une ligne*/
                        fgets(ligne, LIMIT_CHAR*2+1, liste);

                        lg_wd_list = get_word(ligne, wd_in_list, 0);

                        /*Vérifier que le premier mot n'est pas #!*/
                        if(!cmp_word("#!", wd_in_list, lg_wd_list)){
                            strcat(data, ligne);/*Sinon ajouter la ligne au data*/
                        }
                    }while(cmp_word("#!", wd_in_list, lg_wd_list));/*fin de la boucle*/
...
int send_mail_LIST(char *id_nom){
	FILE* liste = NULL; /*pointeur sur le fichier*/
	char ligne[LIMIT_CHAR*2+1] = ""; /*chaine de char accueillant la ligne courante dans le fichier*/
	...
	char id_mail[LIMIT_CHAR]; /*premier mot*/
	char user_from[LIMIT_CHAR]; /*nom de l'expéditeur (2e ligne)*/
	...
	if(liste != NULL){ /* si l'ouverture s'est correctement passée*/
		while(!OK && fgets(ligne, LIMIT_CHAR*2+1, liste) != NULL){ /* parcours du fichier liste*/
            ...
            if(cmp_word("#!", wd_in_list, lg_wd_list)){ /*On vérifie que c'est bien une entête qui suit*/
                ...
                fgets(ligne, LIMIT_CHAR*2+1, liste); /*récupération seconde ligne : FROM*/
                numero_ligne+=1; /*incrementation du numero de ligne*/

                lg_wd_list = get_word(ligne, user_from, 1); /*recuperation nom de l'expediteur*/

                fgets(ligne, LIMIT_CHAR*2+1, liste); /*récupération troisieme ligne : TO*/
                numero_ligne+=1; /*incrementation du numero de ligne*/

                lg_wd_list = get_word(ligne, wd_in_list, 1); /*recuperation nom du destinataire*/


                if(cmp_word(id_nom, wd_in_list, lg_wd_list)){/*Si les noms correspondent*/
...
int traitement_HAVE_LIST(char *id, char *arg){
	...
	if(liste != NULL){ /* si l'ouverture s'est correctement passée*/
		...
		if(OK){ /*si un identifiant a été trouvée*/
            ...
            if(arg == '0') /*Si l'utilisateur veut une liste de message*/
                send_mail_LIST(id_nom);
            else   /*Si il veut un mail en particulier*/
                send_mail_PARTICULAR(arg);
...
int traitement_SEND(char *message){
	/*Une requete SEND doit enregistrer dans la base de données le message reçu*/
	/*Remarque : il aurait été plus optimisé, en envoyant l'id de session et non pas le
	pseudo du destinataire, il y a donc une FAILLE de sécurité*/
    char id_receiver[LIMIT_CHAR] = "";
    int lg_receiver = 0;


    /*On récupère le nom du destinataire*/
    lg_receiver = get_word(message, id_receiver, 1);/* Deuxieme mot dans le SEND*/
	...
	if(liste != NULL){ /* si l'ouverture s'est correctement passée*/
		while(!OK && fgets(ligne, LIMIT_CHAR*2+1, liste) != NULL){ /* parcours du fichier liste*/
			...
			if(cmp_word(id_receiver, wd_in_list, lg_wd_list)){/*Si les id correspondent*/
				OK = 1;
				printf("SEND: Destinataire trouve.\n");
			}
        ...
        if(OK){
            OK = 0; /*init variable*/
            /*Création d'un ID*/
            /*Incrémental -> nécessite la lecture dans le fichier (parcours)*/
            liste = fopen("liste_message", r);

            if(liste != NULL){
                while(!OK && fgets(ligne, LIMIT_CHAR*2+1, liste) != NULL){ /* parcours du fichier liste*/
                    lg_wd_list = get_word(ligne, wd_in_list, 0);

                    if(cmp_word(wd_in_list, "#!", lg_wd_list)) /*Si on a un marqueur mail*/
                        numero_ligne+=1; /*incrementation du numero de ligne*/
            ...
            /*a verifier*/
            if(numero_ligne < 10)
                sprintf(id_mail, "0000000%d", numero_ligne);
            else if(numero_ligne < 100)
                sprintf(id_mail, "000000%d", numero_ligne);
            else if(numero_ligne < 1000)
                sprintf(id_mail, "00000%d", numero_ligne);
            else if(numero_ligne < 10000)
                sprintf(id_mail, "0000%d", numero_ligne);
            else if(numero_ligne < 100000)
                sprintf(id_mail, "000%d", numero_ligne);
            else if(numero_ligne < 1000000)
                sprintf(id_mail, "00%d", numero_ligne);
            else if(numero_ligne < 10000000)
                sprintf(id_mail, "0%d", numero_ligne);
            else
                sprintf(id_mail, "%d", numero_ligne);

            /*Le destinataire existe, écriture dans le fichier.*/
            liste = fopen("liste_message", a); /*On ouvre le fichier en ajout à la fin*/
            /*Il nous faut un id_mail*/
            if(liste != NULL){
                fputs("\n#! ", liste); /*on place un marqueur*/
                fputs(id_mail, liste);
                fputs("\n", liste);

                /*MARQUEUR_ARRET : finir et écrire les data*/
